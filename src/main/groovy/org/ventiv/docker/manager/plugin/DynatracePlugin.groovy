...
class DynatracePlugin implements CreateContainerPlugin {
    ...
    public static final DYNATRACE_EXCLUDE               = "DYNATRACE_EXCLUDE";
    ...
    void doWithServiceInstance(ServiceInstance serviceInstance) {
        ...
        if (env.containsKey(DYNATRACE_PATH) && env.containsKey(DYNATRACE_SERVER) && env.containsKey(DYNATRACE_NAME)) {
            ...
            // Handle the 'exclude' classes.  Loop thru any environment variables that start with "DYNATRACE_EXCLUDE_*" and add them.
            String excludeString = ",exclude=" +
                env.findAll { it.getKey().startsWith(DYNATRACE_EXCLUDE) }.collect { k, clazz ->
                    String classPath = clazz.replace('.', '/');
                    env.remove(k);

                    if (!clazz.startsWith("contains") && !clazz.startsWith("starts") && !clazz.startsWith("ends"))
                        return "starts:" + classPath;

                    return classPath;
                }.join(";");
            if (excludeString == ",exclude=")
                excludeString = "";

            String dynaTraceAgent = " -agentpath:${env[DYNATRACE_PATH]}=name=${env[DYNATRACE_NAME]},server=${env[DYNATRACE_SERVER]}${overrideHostName}${excludeString}"
